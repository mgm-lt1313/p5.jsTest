...
let angerG = 0; // angerの値を保存する変数
let sadG = 0; // sadの値を保存する変数
let disgustedG = 0; // disgustedの値を保存する変数
let surprisedG = 0; // surprisedの値を保存する変数
let fearG = 0; // happinessの値を保存する変数

let happyImg; // 追加: 画像変数

function preload() {
  happyImg = loadImage('happy.png'); // 追加: 画像の読み込み
}
...
class Firework {
  ...
  show() {
    if (!this.exploded) {
      imageMode(CENTER);
      image(happyImg, this.firework.pos.x, this.firework.pos.y, 30, 30); // 追加: happyImgを表示
    }
